...
```
cargo run --release --example csq-tool -- info recording.csq
```

| Command  | Purpose                                                                   |
| -------- | ------------------------------------------------------------------------- |
| `info`   | camera, calibration, timing, GPS                                          |
| `frames` | per-frame temperature statistics (`--tolerant` to salvage damaged frames) |
| `export` | frames as PNG                                                             |
| `scrub`  | evenly spaced thumbnails, using seeks                                     |
| `video`  | encode to MP4 by piping through `ffmpeg`                                  |
| `probe`  | temperature of a single pixel                                             |
| `csv`    | one frame's temperatures as CSV                                           |

`video` needs `ffmpeg` on `PATH`; nothing else does.
